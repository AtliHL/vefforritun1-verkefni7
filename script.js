...
function start() {
    play();
    while(true){
        if(confirm("Viltu spila annann leik?")){
            play();
        } else {
            getResults();
            break;
...
function play() {
    ...
    while(spila == true) {
        ...
        var tala = parseGuess(input);
        var svar = getResponse(tala, random);
        fjöldiGiska++;
        alert(svar);
        if(svar == 'Rétt'){
            games.push(fjöldiGiska);
            break;
        }
    }
}
...
function getResults(){
    if(games.length == 0) {
        ...
        const averageGuesses = calculateAverage();
        alert("Þú spilaðir " + games.length + " leiki\nMeðalfjöldi ágiskana var " + averageGuesses);
    }
}
